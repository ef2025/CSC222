...
*/
public class RemoveTheSmallestSolution {
    ...
    public static int[] removeMostDullThing(int[] things) {
        if (things.length == 0)
            return new int[0];
        int[] modifiedArray = new int[things.length - 1]; // holds array with the most dull thing removed
        int lowestRatingIndex = 0;
        for (int i = 0; i < things.length; i++) {
            if (things[i] < things[lowestRatingIndex]) {
                lowestRatingIndex = i;
            }
        }
        int j = 0; // modified array index
        for (int i = 0; i <  things.length; i++) {
            if (i == lowestRatingIndex)
                continue;
            modifiedArray[j] = things[i];
            j++;
        }
